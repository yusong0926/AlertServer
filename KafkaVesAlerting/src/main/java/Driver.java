...
{
    static private Logger logger = Logger.getLogger(Driver.class);
    static public volatile int target = 0;
        ...
        try {
            ...
            while (true) {
                ...
                for (ConsumerRecord<String, String> record : records) {
                    ...
                    if (type.equals("enodeb") && map.get("profile").equals("video-slice")) {
                        if (alert(map, totalCapacity)) {
                            logger.info("alert enodeb stats: " + map.toString());
                            if (this.target == 1){
                                logger.info("*********Sending Alert to CORD************" + target);
                                sendAlertToONOS(map);
                            }else{
                                logger.info("*********Sending Alert to ONAP************" + target);
                                sendAlertToONAP(map);
                             }
                        }
                    }
    ...
    public boolean alert(Map<String, String> map, int totalCapacity) {
    ...
    public void sendAlertToONOS(Map<String, String> map){
    ...
    public void sendAlertToONAP(Map<String, String> map) {
        //logger.info("start sending alert");
        //logger.debug("alerting events: " + map.toString());
        Date dNow = new Date( );
